...
track <- readRDS("tracked_seqs.rds")
salsa <- as.data.frame(rowSums(phylo_clean_decontam_decontam@otu_table))
queso <- cbind(track, salsa)
colnames(queso)[5] <- "decontam.reads"

# Copied some steps from before
# make a prettier plot by taking the data
plotData <- queso %>% gather(type, totals, reads.in, filtered, nonchimeras, decontam.reads)

#order the types from raw reads to cleanest
plotData$type <- factor(plotData$type, levels = c("reads.in", "filtered", "nonchimeras", "decontam.reads"))
